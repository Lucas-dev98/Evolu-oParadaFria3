...
if (process.env.NODE_ENV === 'production') {
    app.use(express_1.default.static(path_1.default.join(__dirname, '../frontend/dashboard/build')));
}
...
const dataFilePath = path_1.default.join(__dirname, 'areas-data.json');
const cronogramaFilePath = path_1.default.join(__dirname, 'cronograma-data.json');
// Função para carregar dados de cronograma
const loadCronogramaFromFile = () => {
    try {
        if (fs_1.default.existsSync(cronogramaFilePath)) {
            const data = fs_1.default.readFileSync(cronogramaFilePath, 'utf8');
...
const saveCronogramaToFile = (data) => {
    try {
        fs_1.default.writeFileSync(cronogramaFilePath, JSON.stringify(data, null, 2));
    }
    catch (error) {
...
let eventAreas = loadDataFromFile();
// Rotas da API
...
app.get('/api/data-status', (req, res) => {
    res.json({
        source: fs_1.default.existsSync(dataFilePath) ? 'file' : 'default',
        filePath: dataFilePath,
        count: eventAreas.length,
        lastModified: fs_1.default.existsSync(dataFilePath)
            ? fs_1.default.statSync(dataFilePath).mtime
            : null,
    });
});
...
app.get('/api/summary', (req, res) => {
    const totalCapacity = eventAreas.reduce((sum, area) => sum + area.capacity, 0);
    const totalOccupancy = eventAreas.reduce((sum, area) => sum + area.currentOccupancy, 0);
    const occupancyRate = ((totalOccupancy / totalCapacity) * 100).toFixed(1);
    const areasByCategory = eventAreas.reduce((acc, area) => {
    ...
    res.json({
        ...
        averageTemperature: (eventAreas.reduce((sum, area) => sum + area.temperature, 0) /
            eventAreas.length).toFixed(1),
        averageHumidity: (eventAreas.reduce((sum, area) => sum + area.humidity, 0) /
            eventAreas.length).toFixed(1),
    });
});
app.get('/api/realtime', (req, res) => {
    ...
    const realtimeData = eventAreas.map((area) => ({
        id: area.id,
        name: area.name,
        currentOccupancy: area.currentOccupancy + Math.floor(Math.random() * 20 - 10),
...
app.post('/api/cronograma/preparacao', (req, res) => {
    try {
        ...
        const preparacaoIndex = cronogramaData.fases.findIndex((f) => f.id === 'preparacao');
        ...
        const atividadesPreparacao = atividades.map((ativ) => ({
            ...ativ,
            fase: 'preparacao',
        }));
        // Remover atividades antigas de preparação e adicionar novas
        cronogramaData.atividades = cronogramaData.atividades.filter((ativ) => ativ.fase !== 'preparacao');
        ...
        cronogramaData.metadata = {
            ...
            progressoGeral: Math.round(cronogramaData.fases.reduce((acc, f) => acc + f.progress, 0) /
...
app.get('/api/cronograma/atividades/:faseId', (req, res) => {
    try {
        ...
        const atividadesFase = cronogramaData.atividades.filter((ativ) => ativ.fase === faseId);
...
app.get('/api/cronograma/estatisticas', (req, res) => {
    try {
        ...
        const stats = {
            ...
            fasesCompletas: cronogramaData.fases.filter((f) => f.status === 'completed').length,
            fasesEmAndamento: cronogramaData.fases.filter((f) => f.status === 'in-progress').length,
            fasesPendentes: cronogramaData.fases.filter((f) => f.status === 'not-started').length,
            atividadesCriticas: cronogramaData.atividades.filter((a) => a.isCritical)
                .length,
            atividadesAtrasadas: cronogramaData.atividades.filter((a) => a.status === 'delayed').length,
...
app.get('/api/csv/backup/:tipo', async (req, res) => {
    try {
        ...
        const frontendPublicPath = path_1.default.join(__dirname, '../../frontend/dashboard/public', fileName);
        if (fs_1.default.existsSync(frontendPublicPath)) {
            const csvContent = fs_1.default.readFileSync(frontendPublicPath, 'utf-8');
...
app.post('/api/csv/upload/:tipo', upload.single('csvFile'), async (req, res) => {
    try {
        const { tipo } = req.params;
        ...
        const frontendPublicPath = path_1.default.join(__dirname, '../../frontend/dashboard/public', fileName);
        const frontendBuildPath = path_1.default.join(__dirname, '../../frontend/dashboard/build', fileName);
        // Fazer backup do arquivo atual (se existir)
        if (fs_1.default.existsSync(frontendPublicPath)) {
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
            const backupPath = path_1.default.join(__dirname, '../../frontend/dashboard/public', `backup-${tipo}-${timestamp}.csv`);
            fs_1.default.copyFileSync(frontendPublicPath, backupPath);
            console.log(`💾 Backup salvo: backup-${tipo}-${timestamp}.csv`);
        }
        // Salvar novo arquivo
        fs_1.default.writeFileSync(frontendPublicPath, csvContent, 'utf-8');
        console.log(`✅ Arquivo salvo: ${frontendPublicPath}`);
        // Salvar também na pasta build (se existir)
        if (fs_1.default.existsSync(path_1.default.dirname(frontendBuildPath))) {
            fs_1.default.writeFileSync(frontendBuildPath, csvContent, 'utf-8');
        ...
        const stats = {
            ...
            backupCreated: fs_1.default.existsSync(frontendPublicPath),
        };
        res.json({
            success: true,
...
app.post('/api/csv/validate/:tipo', upload.single('csvFile'), async (req, res) => {
    var _a;
    try {
        const { tipo } = req.params;
        if (!req.file) {
        ...
        const csvContent = req.file.buffer.toString('utf-8');
        const lines = csvContent.trim().split('\n');
        const headers = ((_a = lines[0]) === null || _a === void 0 ? void 0 : _a.split(',')) || [];
...
app.get('/api/csv/status', async (req, res) => {
    try {
        const publicPath = path_1.default.join(__dirname, '../../frontend/dashboard/public');
        const files = [
...
if (process.env.NODE_ENV === 'production') {
    app.get('*', (req, res) => {
        res.sendFile(path_1.default.join(__dirname, '../frontend/dashboard/build/index.html'));
    });
}
app.listen(PORT, () => {
