...
export class AIAnalysisService {
  ...
  private apiKey: string = process.env.REACT_APP_GOOGLE_GEMINI_API_KEY || ''; // Chave do arquivo .env
  ...
  async analisarComGeminiDireto(
    ...
    const dadosFormatados = this.formatarDados(cronograma, categorias, resumo);
  ...
  private formatarDados(cronograma: any, categorias: any, resumo: any) {
    return {
  ...
  private async analisarComGemini(dados: any): Promise<AIAnalysis> {
    if (!this.apiKey) {
      this.apiKey = process.env.REACT_APP_GOOGLE_GEMINI_API_KEY || '';
    }
...

{
  ...
  "predicoes": {
    ...
    "recursosEmRisco": ["Cronograma", "Recursos"]
  }
}
`;
    ...
    try {
      ...
      console.error('❌ Erro na requisição do Gemini:', error);
      console.log('🔄 Usando análise local como fallback...');
      return this.analiseLocal(dados);
    }
  }
  ...
  private calcularPontuacao(dados: any): number {
    let pontuacao = dados.concluidas || 0;
